...
WORK = ROOT.parent / ".dogg-rapp1-v2-test-work"
BRIDGE1_PINS = {
    "BRIDGE.md": "a9fcec13f7fa7615f37699f41f97e51ca275022f0b1e240bd9cf652769cc17f8",
    "tools/rapp1_bridge.py": "b9440fe6fc8566c72154828ac276bfae19004baefe3b4797108088d8a58e1b0d",
    "tools/rapp1_recover.mjs": "9d1a06754b9c6fc08be14ca451bea2762973da09f1d244f5003447e64a07672f",
    "tools/rapp1_bridge_vendor/rapp.py":
        "d11ad216fb4cee7a2326d870c934876ecf67fb5af5df876f1866913a55ae3c38",
    "tests/test_rapp1_bridge.py":
        "0f28b9714af5bb5a3282c07c8bdfff4ccab099a1a9547a64f7f686e50fa353bf",
}
NATIVE_PINS = {
    "tools/chainio.py": "9f9aec689112fcf0408dcd564ac0af4f974b0f6ce8aca18b0df3ec3ffd41e46d",
    "tools/verify_thread.py": "4894ad22fb6df5fe98c6417155368ef7fd3b97bd5f224768b73d442b8e59f513",
}
...
class Bridge2(unittest.TestCase):
    def setUp(self):
        ...
        self.write_chain("ticks", self.ticks)
        self.write_chain("world", self.world)
        self.commit("native genesis through head")
        self.initial_native_commit = self.revision
        B.generate_key(self.key, self.repo)
        initialized = B.initialize(
            self.repo, self.revision, "world", B.CANONICAL_REF,
            "local", "dogg-bridge", "rapp1-bridge-v2/registry.json",
            self.state, self.registry_candidate, self.key,
        )
    ...
    def make_world(self, ticks, note="world"):
        frames = []
        for seq, tick in enumerate(ticks):
            ...
            head = frames[-1] if frames else None
            frames.append(B.NATIVE_RAPP.build_frame(
                "world.snapshot", "world:@kody-w/dogg", seq,
    ...
    def append(self, outputs=None, revision=None, checkpoint=None,
               key=None, anchor=None):
        return B.append(
            self.repo, revision or self.revision, B.CANONICAL_REF,
            checkpoint or self.registry_checkpoint, self.state,
            outputs or [self.output], key or self.key,
    ...
    def verify(self, output=None, revision=None, checkpoint=None, anchor=None):
        return B.verify_command(
            self.repo, revision or self.revision, B.CANONICAL_REF,
            checkpoint or self.registry_checkpoint, self.state,
            output or self.output, self.anchor if anchor is None else anchor,
        )
    ...
    @staticmethod
    def source_tuple(frame, chain_path, raw):
        return {
    ...
    def test_signed_profile_refuses_null_wrong_kid_and_wrong_key_algorithm(self):
        ...
        B.generate_key(other_key, self.repo)
    ...
    def test_cli_ref_feature_branch_and_origin_alias_refuse(self):
        ...
        with self.assertRaisesRegex(ValueError, "exact Git top-level"):
            B.check_git_source(
                self.repo / "world", self.revision, B.CANONICAL_REF
            )
        with self.assertRaisesRegex(ValueError, "exact Git top-level"):
            B.append_paths(self.repo / "world", self.state, [self.repo / "output"])
    ...
    def test_append_idempotent_retry_and_candidate_bytes_reused(self):
        native_before = {
            ...
            for chain in ("ticks", "world")
            for path in (self.repo / chain).rglob("*") if path.is_file()
        }
        self.append()
        ...
        world = B.NATIVE_RAPP.build_frame(
            "world.snapshot", "world:@kody-w/dogg", 3,
        ...
        for relative, raw in native_before.items():
    ...
    def test_concurrent_stale_append_is_serialized_and_refused(self):
        ...
        newer_args = (
            self.repo, self.revision, B.CANONICAL_REF, self.registry_checkpoint,
            self.state, [self.output], self.key, self.anchor,
        )
        stale_args = (
            old_repo, old_revision, B.CANONICAL_REF, self.registry_checkpoint,
            self.state, [self.output], self.key, self.anchor,
        )
    ...
    def test_accepted_controller_frame_and_source_metadata_are_immutable(self):
        ...
        changed_payload["native_source"]["storage_path"] = "world/1.json"
    ...
    def test_wrong_anchor_and_private_key_refuse(self):
        with mock.patch.dict(os.environ, {
            ...
            with self.assertRaisesRegex(ValueError, "signing configuration absent"):
                B.append(
                    self.repo, self.revision, B.CANONICAL_REF,
                    self.registry_checkpoint, self.state, [self.output], None, self.anchor,
                )
    ...
    def test_private_key_creation_and_every_use_are_hardened(self):
        ...
        with mock.patch.object(B.os, "open", side_effect=observe_open), \
            ...
            B.generate_key(generated, self.repo)
    ...
    def test_outputs_are_disjoint_before_any_write(self):
        ...
        for outputs in ([self.state], [self.state / "nested-output"]):
            with self.subTest(outputs=outputs):
                with self.assertRaisesRegex(ValueError, "controller state"):
                    B.append(
                        self.repo, self.revision, B.CANONICAL_REF,
                        self.registry_checkpoint, self.state, outputs,
        ...
        with self.assertRaisesRegex(ValueError, "controller state|symlinks forbidden"):
            B.append(
                self.repo, self.revision, B.CANONICAL_REF,
                self.registry_checkpoint, self.state, [alias],
                self.key, self.anchor,
            )
        self.assertFalse(lock.exists())
        alias.unlink()
        ...
        with self.assertRaisesRegex(ValueError, "controller state"):
            B.append_paths(self.repo, state, [state.parent])
        with self.assertRaisesRegex(ValueError, "pairwise disjoint"):
            B.append_paths(
                self.repo, state, [layout / "one", layout / "one" / "nested"]
            )
        with self.assertRaisesRegex(ValueError, "pairwise disjoint"):
            B.append_paths(self.repo, state, [layout / "one", layout / "one"])
        ...
        with self.assertRaisesRegex(ValueError, "state and registry output"):
            B.initialize(
                self.repo, self.revision, "world", B.CANONICAL_REF,
                "local", "overlap", "registry.json", nested_state,
                nested_registry, self.key,
            )
    ...
    def test_case_unicode_and_symlink_directory_aliases_refuse(self):
        ...
        with self.assertRaisesRegex(ValueError, "controller state"):
            B.append_paths(self.repo, state, [alias / "state" / "future"])
                ...
                with mock.patch.object(B, "range", guarded_range, create=True):
                    ...
                    with self.assertRaisesRegex(
                        ...
                        B.recover_output_pending(
                            self.repo, self.revision, B.CANONICAL_REF,
                            self.state, self.output, B.load_config(self.state)[1],
                        )
                pending_path.unlink()
    ...
    def test_partial_output_layout_refuses_unexpected_files_and_symlinks(self):
        ...
        with self.assertRaisesRegex(ValueError, "unexpected bridge output entries"):
            B.recover_output_pending(
                self.repo, self.revision, B.CANONICAL_REF, self.state,
                unexpected, B.load_config(self.state)[1],
            )
        ...
        with self.assertRaisesRegex(ValueError, "not a real directory"):
            B.recover_output_pending(
                self.repo, self.revision, B.CANONICAL_REF, self.state,
                wrong_type, B.load_config(self.state)[1],
            )
        ...
        with self.assertRaisesRegex(ValueError, "symlinks forbidden"):
            B.recover_output_pending(
                self.repo, self.revision, B.CANONICAL_REF, self.state,
                linked, B.load_config(self.state)[1],
            )
    ...
    def test_native_commit_path_raw_rollback_fork_and_genesis_refuse(self):
        ...
        with self.assertRaisesRegex(ValueError, "source path changed|genesis changed"):
            self.append()
        config_path.write_bytes(config_raw)

        self.write_chain("world", self.world[:2])
        ...
        forked[2] = B.NATIVE_RAPP.build_frame(
            "world.snapshot", "world:@kody-w/dogg", 2, forked[2]["utc"],
            {**forked[2]["payload"], "note": "fork"},
            forked[1]["payload_hash"],
        )
        self.write_chain("world", forked)
        ...
        self.write_chain("world", changed)
    ...
    def test_every_accepted_frame_has_exact_historical_native_provenance(self):
        copied = self.repo / "world" / "historical-copy.json"
        copied.write_bytes((self.repo / "world" / "1.json").read_bytes())
        self.commit("add misleading historical native copy")
        self.append()

        def use_copy(kind, payload):
            payload["native_source"]["storage_path"] = "world/historical-copy.json"
            return kind, payload
    ...
    def test_every_accepted_frame_has_exact_historical_tick_provenance(self):
        copied = self.repo / "ticks" / "historical-copy.json"
        copied.write_bytes((self.repo / "ticks" / "1.json").read_bytes())
        self.commit("add misleading historical tick copy")
        self.append()
    ...
    def test_source_bytes_and_modes_are_bound_to_the_commit_tree(self):
        self.write_chain("world", self.world, epoch_size=2, sealed_epochs=1)
        committed = self.commit("seal source for immutable tree test")
        committed_epoch = B.git_blob(
            self.repo, committed, "world/epochs/0.jsonl"
        )
        ...
        def mutate_after_git_check(*args, **kwargs):
            ...
            if not mutated:
                mutated = True
                epoch = self.repo / "world" / "epochs" / "0.jsonl"
                epoch.write_bytes(epoch.read_bytes().replace(b"world-1", b"mutable"))
                (self.repo / "world" / "2.json").write_bytes(b"not native JSON\n")
            return result
        ...
        self.git("checkout", "--", "world")
        os.chmod(self.repo / "world" / "2.json", 0o755)
        mode_commit = self.commit("make native source executable")
        self.assertIn(
            "100755",
            self.git("ls-tree", mode_commit, "world/2.json"),
        )
    ...
    def test_sealed_epoch_source_is_exact_opaque_line(self):
        self.write_chain("world", self.world, epoch_size=2, sealed_epochs=1)
        self.commit("seal native world epoch")
        report = self.append()
        self.assertEqual(report["frames_verified"], 3)
        frame1 = json.loads((self.output / "frames" / "1.json").read_bytes())
        source = frame1["payload"]["native_source"]
        self.assertEqual(source["storage_path"], "world/epochs/0.jsonl")
    ...
    def test_sealed_epoch_over_one_mib_is_valid_when_each_record_is_bounded(self):
        ...
        self.write_chain("world", frames, epoch_size=3, sealed_epochs=1)
        epoch = self.repo / "world" / "epochs" / "0.jsonl"
        ...
        for case in cases:
            with self.subTest(case=case):
                ...
                if case == "aggregate":
                    ...
                    head_path = self.repo / "world" / "HEAD.json"
                    ...
                    shutil.rmtree(self.repo / "world" / "epochs")
                    for seq, frame in enumerate(self.world):
                        (self.repo / "world" / f"{seq}.json").write_text(
    ...
    def test_sealed_epoch_structure_and_encoding_refuse(self):
        ...
        cases = {
            ...
            "order": ("native seq gap at world/0", lambda lines: list(reversed(lines))),
    ...
    def test_tick_anchor_stream_kind_and_payload_are_exact_everywhere(self):
        ...
        cases = (
            ...
            ("kind", "tick:@kody-w/global", "world.snapshot", {"tick": 0}, "wrong kind"),
        ...
        for label, stream_id, kind, first_payload, reason in cases:
            with self.subTest(label=label):
                ...
                self.write_chain("ticks", frames)
                invalid = self.commit(f"invalid tick {label}")
                with self.assertRaisesRegex(ValueError, reason):
                    self.append(revision=invalid)
                self.assertFalse(self.output.exists())
                self.git("reset", "--hard", accepted)
    ...
    def test_private_key_never_enters_repository_state_or_outputs(self):
        native_before = {
            path.relative_to(self.repo).as_posix(): digest(path)
            for chain in ("ticks", "world")
            for path in (self.repo / chain).rglob("*") if path.is_file()
        }
        self.append([self.output, self.second_output])
        for root in (self.repo, self.state, self.output, self.second_output):
    ...
    def test_bridge1_native_tools_and_vendor_provenance_are_immutable(self):
        for relative, expected in {**BRIDGE1_PINS, **NATIVE_PINS}.items():

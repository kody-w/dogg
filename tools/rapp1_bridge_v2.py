...
BRIDGE2_SPEC_SHA256 = "5ef75b12bab4b1403908f504b97bab59d78be156f015cf9e8d3919377e747dfa"
...
CONFIG_KEYS = {
    ...
    "stream_id", "trust_anchor", "rapp1_reference", "bridge2_spec_sha256",
}
HEAD_KEYS = {
    "schema", "profile", "registry_seq", "registry_sha256", "registry_checkpoint",
    "source_commit", "chain_path", "native_seq", "native_frame_hash",
    "native_genesis_frame_hash", "rapp_seq", "rapp_frame_hash",
    "rapp_payload_hash", "rapp_utc", "stream_id",
}
...
def check_git_commit(root, revision, main_ref):
    ...
    require(remotes == [SOURCE_REPOSITORY],
...
def check_git_source(root, revision, main_ref):
    root = check_git_commit(root, revision, main_ref)
...
def verify_native_chain(root, revision, chain_path, main_ref, cache=None):
    chain_path = safe_relative(chain_path, "native chain path")
    root = check_git_commit(root, revision, main_ref)
    key = (str(root), revision, chain_path, main_ref)
...
def provenance_for(chain, seq, main_ref, cache):
    frame = chain["frames"][seq]
    payload = frame.get("payload")
    ...
    ticks = verify_native_chain(
        chain["root"], chain["revision"], "ticks", main_ref, cache=cache
    )
...
def validate_source_tuple(value):
    require(isinstance(value, dict) and set(value) == SOURCE_KEYS, "native source tuple key set")
    require(value["repository"] == SOURCE_REPOSITORY, "native source repository")
...
def key_path(value, source_root, forbidden=()):
    ...
    require(not path_within(path, source_root), "private signing key must be outside the repository")
...
def append_paths(source_root, state, outputs):
    ...
    return source_root, state, normalized
...
def initialize(source_root, source_revision, chain_path, main_ref, owner, slug,
               canonical_registry_path, state, registry_out, signing_key):
    ...
    require(not paths_overlap(state, source_root) and
            not paths_overlap(registry_out, source_root),
            "initial state and registry output must be disjoint from the repository")
    ...
    private_key = key_path(signing_key, source_root, (state, registry_out))
    ...
    native, tick_frame, tick_source = provenance_for(chain, 0, main_ref, cache)
    ...
    if tick_source is not None:
        tick_chain = cache[(str(source_root), source_revision, "ticks", main_ref)]
        write_new_or_equal(
...
def validate_head_transition(current, candidate, label):
    ...
    if candidate["rapp_seq"] == current["rapp_seq"]:
        ...
        require(
            ...
            candidate["native_frame_hash"] == current["native_frame_hash"],
...
def validate_frame_payload(frame, seq, config=None):
    ...
    if payload["tick_frame"] is None:
        require(payload["tick_source"] is None, "tick source without tick_frame")
    else:
        require(isinstance(payload["tick_frame"], str) and
                HEX64.fullmatch(payload["tick_frame"]), "invalid projected tick_frame")
        validate_source_tuple(payload["tick_source"])
        require(payload["tick_source"]["native_frame_hash"] == payload["tick_frame"],
                "projected tick tuple/hash mismatch")
        require(payload["tick_source"]["chain_path"] == "ticks", "projected tick source path")
        require(payload["tick_source"]["native_stream_id"] == "tick:@kody-w/global",
                "projected tick source stream")
        require(payload["tick_source"]["commit"] == native["commit"],
                "projected native and tick sources use different commits")
    return native
...
def verify_candidate_provenance(source_root, candidate, main_ref, current_revision, cache):
    ...
    expected_native, expected_tick_frame, expected_tick = provenance_for(
        historical, native["native_seq"], main_ref, cache
    )
    ...
    if expected_tick is not None:
        git_ancestor(source_root, expected_tick["commit"], current_revision,
                     "candidate tick commit is not an ancestor of current source")
        historical_ticks = verify_native_chain(
            source_root, expected_tick["commit"], "ticks", main_ref, cache=cache
        )
        ...
        ticks = verify_native_chain(
            source_root, current_revision, "ticks", main_ref, cache=cache
        )
...
def recover_controller_pending(source_root, source_revision, main_ref,
                               state, config):
    ...
    for frame, _ in frames:
        verify_candidate_provenance(
            source_root, frame, main_ref, source_revision, cache
        )
    if current is not None:
        git_ancestor(
...
def verify_controller_accepted(source_root, source_revision, main_ref,
                               state, config, head):
    if head is None:
        return
...
def recover_output_pending(source_root, source_revision, main_ref, state,
                           output, config):
...
def _append_locked(source_root, source_revision, main_ref, registry_checkpoint,
    ...
    private_key = key_path(signing_key, source_root, (state, *outputs))
    ...
    prior_head, recovered_state = recover_controller_pending(
        source_root, source_revision, main_ref, state, config
    )
    ...
    verify_controller_accepted(
        source_root, source_revision, main_ref, state, config, prior_head
    )
    ...
    for frame, _ in frames[first_unaccepted:]:
        verify_candidate_provenance(source_root, frame, main_ref, source_revision, cache)
    ...
    while frames[-1][0]["seq"] < target:
        ...
        native, tick_frame, tick_source = provenance_for(chain, seq, main_ref, cache)
        utc = now_utc()
        ...
        if tick_source is not None:
            ticks = verify_native_chain(
                source_root, source_revision, "ticks", main_ref, cache=cache
            )
            write_new_or_equal(
                state / "blobs" / f"{tick_source['raw_sha256']}.bin",
...
def append(source_root, source_revision, main_ref, registry_checkpoint,
           state, outputs, signing_key, explicit_anchor=None):
    source_root, state, outputs = append_paths(source_root, state, outputs)
    with controller_lock(state):
        return _append_locked(
            source_root, source_revision, main_ref, registry_checkpoint,
            state, outputs, signing_key, explicit_anchor,
        )


def verify_command(source_root, source_revision, main_ref, registry_checkpoint,
...
def advance_registry(registry_in, registry_out, signing_key, explicit_anchor, source_root):
    source_root = source_repository_root(source_root)
    private_key = key_path(signing_key, source_root, ())
    anchor = trust_anchor(explicit_anchor)
    ...
    require(not path_within(registry_out, source_root),
            "registry candidate output must be outside the repository")
...
def generate_key(path, source_root):
    source_root = source_repository_root(source_root)
    path = absolute(path)
    require(not path_within(path, source_root), "private key output must be outside the repository")
    with private_directory_descriptor(path.parent) as (_, directory_fd):
        try:
...
def main(argv=None):
    ...
    try:
        if args.command == "generate-key":
            result = generate_key(args.out, args.source_root)
        elif args.command == "init":
            result = initialize(
                args.source_root, args.source_revision, args.chain, args.main_ref,
                args.owner, args.slug, args.canonical_registry_path,
                args.state, args.registry_out, args.signing_key,
            )
        elif args.command == "append":
            result = append(
                args.source_root, args.source_revision, args.main_ref,
                args.registry_checkpoint, args.state, args.out,
                args.signing_key, args.trust_anchor,
            )
        elif args.command == "verify":
            result = verify_command(
                args.source_root, args.source_revision, args.main_ref,
                args.registry_checkpoint, args.state, args.out, args.trust_anchor,
            )
        else:
            result = advance_registry(
                args.registry_in, args.registry_out, args.signing_key,
                args.trust_anchor, args.source_root,
            )
